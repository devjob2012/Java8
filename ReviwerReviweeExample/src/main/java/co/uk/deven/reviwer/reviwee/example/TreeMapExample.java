...
public class TreeMapExample {
	...
	public String printGraph(String key, NodeExample<String> child, int count) {
		if (nodeMap.get(key).getChildren() != null) {
			count++;
			for (NodeExample<String> child1 : nodeMap.get(key).getChildren()) {
	...
	private void createMap() throws FileNotFoundException {
		...
		while (sc.hasNextLine()) {
			...
			if (revieweeNode == null) {
				revieweeNode = new NodeExample<String>(reviewee, reviewerNode);
